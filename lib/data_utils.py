...
def load_data(batch_idx, batch_size, split,
    ...
    for i, img_path in enumerate(batch_imgs):
        ...
        img_array = img_array.transpose((0, 3, 1, 2)) / 255.
        input = input.transpose((0, 3, 1, 2)) / 255.
        #wrong_img = np.copy(input)
        if split == 'train2014' or split == 'val2014':
            input[:, :, center[0] - 16:center[0] + 16, center[1] - 16:center[1] + 16] = 0
        else:
            input[:, :, center[0] - 32:center[0] + 32, center[1] - 32:center[1] + 32] = 0
    ...
    train_set_x = np.asarray(train_set_x * 2. - 1., dtype='float32')
    train_set_y = np.asarray(train_set_y * 2. - 1., dtype='float32')
