# Taken from https://github.com/Newmu/dcgan_code/blob/master/lib/inits.py
import os
import numpy as np
...
def constant(shape, c=0., name=None):
    return sharedX(np.ones(shape) * c, name=name)
...
def He(shape, fan_in, name=None, shared=True):
    rng = np.random.RandomState(1)
    W = rng.normal(0, np.sqrt(2. / fan_in), size=shape)
    return sharedX(W, name=name)