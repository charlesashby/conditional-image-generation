    ...
    def train_model(self):
        ...
        while epoch < n_epochs and not DONE_LOOPING:
            ...
            if validation_loss < best_validation_loss:
                ...
                # Save the model
                print('Saving...')
                self.save('model_ae_64x64_mask.pkl')
                # -------------
...
if __name__ == '__main__':
    ...
    network.train_model()
    """
    from lib_models.cae_bn import *
    network = AutoEncoder()
    network.build_model()
    network.load('lib_models/model_ae_64x64_mask.pkl')
    t = network.generate()
    ...
    #network.gpu_dataset = ['dwijdiw']
    #print(network.gpu_dataset)
    #network.generate_image(1)
    #test = network.train_fn(0)
    #print('test', test)
    #val = network.test_fn(0)
    #print('valid', val)
    #test = network.generate_image(1)

    #print('test 0 :', test[0].shape)
    #print('test 1 :', test[1].shape)
    #Image.fromarray(test[0][0], 'RGB')
    #Image.fromarray(test[1][0], 'RGB')
